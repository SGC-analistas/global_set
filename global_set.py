...
if '/home/' in cwd:                                                # inside /home/user directory
    ...
    # --- Set the desired server (13, 58, 222, or default - 232) ---
        # Condition: server xxx.13
    if server == '13':
    
        default(lines)                                             # set the global.cfg as default
        set_server('xxx.13', lines)                                # set xxx.13          
        overwrite(file_path, lines)                                # overwrite the global.cfg file with the changes
        
        # Condition: server xxx.50
    elif server == '50':
    
        default(lines)                                             # set the global.cfg as default (xxx.232)
        set_server('xxx.50', lines)                                # set xxx.50
        overwrite(file_path, lines)                                # overwrite the global.cfg file with the changes    
    
        # Condition: server xxx.58
    elif server == '58':
    
        default(lines)                                             # set the global.cfg as default (xxx.232)
        set_server('xxx.58', lines)                                # set xxx.58
        overwrite(file_path, lines)                                # overwrite the global.cfg file with the changes
        os.system(cmd)                                             # Run the scolv

        # Condition: server xxx.222
    elif server == '222':
    
        default(lines)                                             # set the global.cfg as default (xxx.232)
        set_server('xxx.222', lines)                               # set xxx.222          
        overwrite(file_path, lines)                                # overwrite the global.cfg file with the changes
            
        # Condition: default - server xxx.232
